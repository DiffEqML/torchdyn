...
from torchdyn.numerics.solvers.ode import (
...
def odeint(
    ...
    if t_span[1] < t_span[0]:  # time is reversed
        if verbose:
            warn(
                "You are integrating on a reversed time domain, adjusting the vector field automatically"
            )
        f_ = lambda t, x: -f(-t, x)
        t_span = -t_span
    else:
    ...
    if isinstance(solver, Tsitouras45):
        if verbose:
            warn("Running interpolation not yet implemented for `tsit5`")
        interpolator = None
    ...
    # odeint routine with a single t_span for all samples
        ...
        if stepping_class == "fixed":
            ...
            if len(save_at) > 0:
                warn("Setting save_at has no effect on adaptive-step methods")
            return _adaptive_odeint(
                f_,
                k1,
                x,
                dt,
...
def odeint_symplectic(
    ...
    if not hasattr(f, "order"):
        raise RuntimeError(
            "The system order should be specified as an attribute `order` of `vector_field`"
        )
    if isinstance(solver, AsynchronousLeapfrog) and f.order == 2:
        raise RuntimeError(
            "ALF solver should be given a vector field specified as a first-order symplectic system: v = f(t, x)"
        )
    ...
    # odeint routine with a single t_span for all samples
        ...
        if stepping_class == "fixed":
            if (
                atol != odeint_symplectic.__defaults__[0]
                or rtol != odeint_symplectic.__defaults__[1]
            ):
                warn("Setting tolerances has no effect on fixed-step methods")
        ...
        if triggered_events > 0:
            ...
            while niters < max_iters and event_tol < dt_inner:
                with torch.no_grad():
                    ...
                    new_event_states = EventState(
                        [cb.check_event(t_inner + dt_inner, x_) for cb in callbacks]
                    )
                    triggered_events = sum(
                        [
                            (a_ != b_) & (b_ == False)
                ...
                if (
                    triggered_events == 0
                ):  # if no event, advance start point of bisection search
                    x_inner = x_
            ...
            if seminorm[0] == True:
                ...
                error_scaled = error / (
                    atol + rtol * torch.max(x[:state_dim].abs(), x_new[:state_dim].abs())
                )
            else:
                error = x_err
...
def _adaptive_odeint(
    ...
    while t < T:
        ...
        dt = adapt_step(
            dt, error_ratio, solver.safety, solver.min_factor, solver.max_factor, solver.order
        )
    return torch.cat(eval_times), torch.stack(sol)
...
def _fixed_odeint(f, x, t_span, solver, save_at=(), args={}):
    ...
    assert all(
        torch.isclose(t, save_at).sum() == 1 for t in save_at
...
def _jagged_fixed_odeint(f, x, t_span: List, solver):
    """
