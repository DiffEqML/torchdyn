...
def _check_types(sde, solver_sde_type, solver_noise_type):
    if sde.sde_type != solver_sde_type:
        raise ValueError(
            f"SDE's type : {sde.sde_type} and solver type : {solver_sde_type} is different!"
        )
    if sde.noise_type not in solver_noise_type:
        raise ValueError(
            f"Solver only supports noise types of {solver_noise_type} but noise type of {sde.noise_type} is given by SDE"
        )
